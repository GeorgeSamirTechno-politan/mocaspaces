package com.technopolitan.mocaspaces.utilities.customBottomNivigation
...
class CustomBottomNavigation : BottomNavigationView {
    ...
    private var enabledColor = ContextCompat.getColor(context, R.color.accent_color)
    private var textColor by Delegates.notNull<Int>()

    var backgroundShape = Shape.Circle
    private var customBackgroundDrawable = -1
    var circleColor = ContextCompat.getColor(context, R.color.accent_color)
    var darkIcon = false
        set(value) {
            field = value
            updateEnabledColor()
    ...
    private fun buildBackgroundCircle(): ImageView {
        ...
        if (customBackgroundDrawable == -1) {
            val backgroundShapeDrawable = when (backgroundShape) {
                Shape.Circle -> ContextCompat.getDrawable(context, R.drawable.white_circle_shape)
                Shape.RoundedRectangle -> ContextCompat.getDrawable(
                    context,
                    R.drawable.white_rectangle_shape
                )
            }
